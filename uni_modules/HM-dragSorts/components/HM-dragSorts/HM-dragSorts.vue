<template>
	<view class="HM-drag-sort" :style="{'height': ListHeight+'px'}">
				...
				<uni-swipe-action>
					<view class="list color" :class="[listType=='A'?(listSwitch?'show':'hide'):(listSwitch?'hide':'show')]">
						<block v-for="(row,index) in tmplist" :key="index">
							<uni-swipe-action-item :auto-close="false"  :right-options="options" style="overflow: visible !important;" @click="deleteTab(row,index)">
		...
		<view :data-isapph5="isAppH5" :data-islongtouch="isLongTouch" :data-feedbackgeneratorstate = "feedbackGeneratorState" :data-longtouchtime="longTouchTime" :data-listheight="ListHeight"
...
<style lang="scss">
	/deep/ uni-swipe-action-item:last-child .modules {
